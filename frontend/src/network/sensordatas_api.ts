...
export async function fetchSensorDatas(): Promise<SensorData[]> {
    const response = await fetchData("http://localhost:5000/api/sensordata", { method: "GET"});                  //this is the link we setup in the backend to get all of our sensordata, so we use a get request
...
export async function createSensorData(sensordata: SensorDataInput): Promise<SensorData> {
